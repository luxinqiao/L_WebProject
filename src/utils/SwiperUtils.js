var SwipeUtils = {
    ...
    touchStartX: 0,
    transing: false,
    interval: 3000, //轮播间隔（单位：ms）
    animationInterval: 400, //轮播执行动画间隔（单位：ms）
    moveDistance: 50, //需触发轮播的手指滑动x轴最小距离（单位：px）
    ...
    /**
        ...
        @return {String}
     */
    init({$el, imgs, defaultNo, autoPlay, clickCallback}) {
        //提示
        if (imgs.length < 3) {
    ...
    /**
        ...
        @return
     */
    refreshCurrImg: function() {
        var _this = this
        $('.mdSwiper>.swiper-item').each(function(i, val) {
            var no = $(val).attr('no')
        ...
        var $swiperIndex = $('.mdSwiper>.swiper-index')
