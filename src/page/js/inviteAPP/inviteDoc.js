...
$(function(){
    ...
    CommonUtils.setHtmlTitle('邀您加入澜渟医生')
    ...
    if(isFirst) {
        ...
        if (phone == '') {
            ...
            AjaxUtils.ajax_new({
                ...
                success: function(data, textStatus, jqXHR) {
                    if (data.code == 200) {
                        DialogUtils.newTipAlert('优惠券已放入您的账户，打开澜渟医生APP，立即使用！','确定',()=>{
                            if((type=='wechat')){
                                Download_wechat('doc')
                                isFirst = true
                            } else {
                        ...
                        DialogUtils.newConfirm('<span style="display:block;text-align:center;">'+data.msg+'</span><span style="display:block;text-align:center;">是否前往澜渟医生APP？</span>','否',function(){
                            isFirst = true
                        },'是',function(){
                            if((type=='wechat')){
                                Download_wechat('doc')
                        ...
                        $('#dialogCon .button .confirm').css('color',' #26C2C4')
                    }
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
